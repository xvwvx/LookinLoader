...
%ctor{
	...
	@autoreleasepool {
		NSDictionary *pref = [NSDictionary dictionaryWithContentsOfFile:@THEOS_PACKAGE_INSTALL_PREFIX"/User/Library/Preferences/com.todayios-cydia.lookinloader.plist"];
		...
		NSLog(@"LookinLoader selectedApplications:%@ contains %@", selectedApplications, bundleIdentifier);
		if (appEnabled) {
			NSFileManager* fileManager = [NSFileManager defaultManager];
			NSString *libPath = @THEOS_PACKAGE_INSTALL_PREFIX"/Library/Application Support/LookinLoader/LookinServer.framework/LookinServer";
			...
			if([fileManager fileExistsAtPath:libPath]) {
				...
				if (lib) {
					...
					NSLog(@"[+] LookinLoader load failed");
				}
			} else {
				NSLog(@"[+] LookinLoader load failed: 文件不存在");
			}
		}
